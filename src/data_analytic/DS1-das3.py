...
# Bagaimana hubungan antara waktu saat berkendara terhadap kemungkinan kecelakaan?
...
def perform_linear_regression_with_statsmodels(data_frame, weather_col_name, injuries_col_name, csv_filename):
    ...
    try:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'../../doc/data-analytic/DS1-Pertanyaan3.png', dpi=300, bbox_inches='tight')
        plt.show()
...
if __name__ == "__main__":
    ...
    waktu_kejadian = "crash_hour" 
    jumlah_korban = "injuries_total"

    try:
        data_full = pd.read_csv(file_path)

        if waktu_kejadian not in data_full.columns or jumlah_korban not in data_full.columns:
            print(f"Error Kritis: Kolom '{waktu_kejadian}' atau '{jumlah_korban}' tidak ditemukan di CSV '{file_path}'.")
            print(f"Kolom yang tersedia: {data_full.columns.tolist()}")
            exit()
            
        data_selected = data_full[[waktu_kejadian, jumlah_korban]].copy()

        perform_linear_regression_with_statsmodels(data_selected, waktu_kejadian, jumlah_korban, actual_filename)
