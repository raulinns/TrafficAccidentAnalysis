# Dataset 2: Nashville Accidents (2024)
# Pertanyaan Penelitian 2
# Bagaimana hubungan antara jenis kecelakaan terhadap jumlah korban kecelakaan?
...
def perform_linear_regression_with_statsmodels(data_frame, weather_col_name, injuries_col_name, csv_filename):
    ...
    try:
        ...
        plt.title(f'Regresi Linear Dataset 2: "{csv_filename}"', fontsize=15)
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'../../doc/data-analytic/DS2-Pertanyaan2.png', dpi=300, bbox_inches='tight')
        plt.show()
...
if __name__ == "__main__":
    file_path = '../../dataset/transformasi-dataset/nashville_accidents_2024_mapped.csv'
    actual_filename = os.path.basename(file_path)
    jenis_kecelakaan = "Collision Type Description" 
    jumlah_korban = "Number of Injuries"


    try:
        data_full = pd.read_csv(file_path)

        if jenis_kecelakaan not in data_full.columns or jumlah_korban not in data_full.columns:
            print(f"Error Kritis: Kolom '{jenis_kecelakaan}' atau '{jumlah_korban}' tidak ditemukan di CSV '{file_path}'.")
            print(f"Kolom yang tersedia: {data_full.columns.tolist()}")
            exit()
            
        data_selected = data_full[[jenis_kecelakaan, jumlah_korban]].copy()

        perform_linear_regression_with_statsmodels(data_selected, jenis_kecelakaan, jumlah_korban, actual_filename)
